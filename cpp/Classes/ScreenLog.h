...
ScreenLog* ScreenLog::getInstance()
{
    ...
    if (!log) {
        ...
        log->setLevelMask( LL_DEBUG | LL_INFO | LL_WARNING | LL_ERROR | LL_FATAL );
        log->setTimeoutSeconds( 0xFFFF );
        cocos2d::CCDirector::sharedDirector()->setNotificationNode(cocos2d::CCNode::create());
        log->attachToScene(cocos2d::CCDirector::sharedDirector()->getNotificationNode());
    }
    return log;
}
...
void screenLogMessage::createLabel()
{
    float screenHeightPixels = cocos2d::CCDirector::sharedDirector()->getWinSize().height;
    float fontSize =  screenHeightPixels / (float)SCREENLOG_NUM_LINES - 1;
    m_label = cocos2d::Label::create(m_text.c_str(), m_layer->m_fontFile, fontSize);
